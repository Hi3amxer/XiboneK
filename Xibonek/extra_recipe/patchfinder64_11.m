...
//  Copyright © 2017 @Hi3amxer. All rights reserved.
//
//  iOS 11 stuff by John Lewis @Hi3amxer
//
...
{
    size_t last, scan = 0;
    size_t bad_char_skip[P_UCHAR_MAX + 1]; /* Officially called:
                                          * bad character shift */
     ...
     */
    for (scan = 0; scan <= P_UCHAR_MAX; scan = scan + 1)
...
#define INSN_STR_1  0xF9016128, 0xFFFFFFFF
#define INSN_STR_2  0xF9042128, 0xFFFFFFFF
#define INSN_STR_3  0xF9036128, 0xFFFFFFFF
#define INSN_STR_4  0xF9078128, 0xFFFFFFFF

#define INSN_ADD  0x8B090100, 0xFFFFFFFF
#define INSN_CALL 0x94000000, 0xFC000000
#define INSN_B    0x14000000, 0xFC000000
#define INSN_CBZ  0x34000000, 0xFC000000

#define INSN_CBNZ_1 0x35000D78, 0xFFFFFFFF
#define INSN_CBNZ_2 0x35000DB8, 0xFFFFFFFF
#define INSN_CBNZ_3 0x35000EF8, 0xFFFFFFFF

#define INSN_ADRP_1 0xD0002317, 0xFFFFFFFF
#define INSN_ADRP_2 0xD0002236, 0xFFFFFFFF
#define INSN_ADRP_3 0x90002294, 0xFFFFFFFF
#define INSN_ADRP_4 0xD0002234, 0xFFFFFFFF
...
find_gPhysBase(void)
{
    ...
    if (!ref) {
        printf("[ERROR]: could not find 'pmap_map_high_window_bd'\n");
        return 0;
    }
    ref -= kerndumpbase;
    ...
    if (!ret) {
        ...
        // this will get us the first RET
        ret = step64(kernel, ref, 381, INSN_RET);
        
        if(!ret)
            return 0;
        
        // get the ADD that follows
        addr_t add = step64(kernel, ret, 25, INSN_ADD);
        
        if(!add)
            return 0;
        
        // get the RET right after add
        ret = step64(kernel, add, 5, INSN_RET);
        
        if(!ret)
            return 0;
    }
    extern uint64_t kaslr_slide;
    printf("ret is at: %llx\n", (uint64_t)ret+kerndumpbase-kaslr_slide);

    for(int i = 0; i < 500; i++) {

        val = calc64(kernel, ref, ret, i);
        printf("vnode possibly at: %llx: %d\n", val+kerndumpbase-kaslr_slide, i);
...
find_trustcache(void)
{
    ...
    if (!ref) {
        ...
        // try the alternative method since iOS 11 doesn't have the str
        ref = find_strref("com.apple.MobileFileIntegrity", 1, 1);
        printf("com.apple.MobileFileIntegrity: %llx\n", (uint64_t)ref);
        
        ref -= kerndumpbase;
        
        addr_t first_bl = step64(kernel, ref, 56, INSN_CALL);
        if (!first_bl)
            return 0;
        

        addr_t str = step64(kernel, first_bl, 25, INSN_STR_1);

        if (!str) {
            
            str = step64(kernel, first_bl, 25, INSN_STR_2);
            
            if(!str) {
                
                str = step64(kernel, first_bl, 25, INSN_STR_3);

                if(!str) {

                    str = step64(kernel, first_bl, 25, INSN_STR_4);
        ...
        NSLog(@"cache: final str is at: %llx\n", (uint64_t) str);
        addr_t target_bl = step64(kernel, str, 5, INSN_CALL);
        if (!target_bl)
            return 0;
        
        NSLog(@"cache: target_bl is at: %llx\n", (uint64_t) target_bl);

        func = follow_call64(kernel, target_bl);
        if (!func)
            return 0;
